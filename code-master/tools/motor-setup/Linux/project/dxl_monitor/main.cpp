...
static inline void chomp(char* str, int* len){
    if(str[*len - 1] == '\n')
    {
        --*len;
        str[*len] = 0;
...
int main()
{
	...
	signal(SIGINT, &sighandler);
        ...
        char pid[20];
        FILE * fp = fopen("/tmp/cm370lock","r");
        if (fp != NULL)
        {
            fread(pid, 1, 10, fp);
            if (!strcmp(pid, "") && 0 == kill(atoi(pid), 0))
            {
                printf("Es scheint bereits ein Programm auf das CM-370 zuzugreifen, bitte beende dieses!\n");
                exit(1);
            }
            fclose(fp);
        }
        FILE *fp2 = fopen("/tmp/cm370lock","w");
        sprintf(pid,"%d",(int) getpid());
        fwrite(pid, 1, strlen(pid), fp2);
        fclose(fp2);
