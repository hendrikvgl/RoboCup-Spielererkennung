...
public:
    ...
    /**
     * This is an experimental function. It tries to calculate whether the robot is statically stable.
     * If not, it tries to formulate a fitting kinematic task to avoid falling.
     * This method is still experimental!!
     * \param robot: The robot used for calculation
     * \param foot_size: A matrix representing the foots dimensions
     * \param security_offset: A minimal offset the centre of gravity has to be inside the calculated support polygon
     */
    static void keep_robot_stable(KRobot& robot, const Eigen::Matrix2d& foot_size, double security_offset=5, int max_it=100);
    static void keep_robot_stable_with_target(KRobot& robot, const Eigen::Matrix2d& foot_size, Robot::Kinematics::JointIds endpoint, const Eigen::Vector3d& target, double security_offset=5, int max_it=100);
