...
cpdef keep_robot_stable(Robot robot, int max_it=-1):
    print "Try to keep Robot Stable"
    if max_it == -1:
        keep_robot_stable_(deref(robot.robot), foot_size)
    else:
        keep_robot_stable_(deref(robot.robot), foot_size, max_it)

cpdef keep_robot_stable_with_target(Robot robot, int endpoint, np.ndarray target, int max_it=-1):
    print "Try to keep Robot Stable"
    if max_it == -1:
        keep_robot_stable_with_target_(deref(robot.robot), foot_size, int_to_JointID(endpoint), Vector3d(target[0], target[1], target[2]))
    else:
        keep_robot_stable_with_target_(deref(robot.robot), foot_size, int_to_JointID(endpoint), Vector3d(target[0], target[1], target[2]), max_it)
