#-*- coding:utf-8 -*-
"""
Record-Script Functions.
^^^^^^^^^^^^^^^^^^^^^^^^

This is the oldest class of the record-script,
remaining from a time long before we built an urwid-ui
Anyone interested in software-archeology? This is the place to come.

On the ruins of the former record-script we built the record-ui
so this module is still used, maintained and extended.
In an MVC-Architecture, this would be the Controller.

The ui and recorder are somewhat seperated,
keeping in mind that someone might, at some point, want to build
another interface to the recorder.
However the urwid-ui is by no means free of logical-function.
For example the urwid-ui takes care of the data validation so
building a new record-interface would still be a bothersome task. Sorry.

.. moduleauthor:: Timon Giese <timon.giese@bit-bots.de>
.. moduleauthor:: Marc Bestmann

History
-------

    * 2014-06-30 History angelegt, Doku erweitert
"""
import json
import logging
import datetime
...
from bitbots.motion import animation
from bitbots.util import find_animation, find, Joints
from bitbots.ipc import NotControlableError
from bitbots.record.util import list_select, index_select, property_proxy
...
class Recorder(object):
    ...
    def __init__(self, ipc, gui, logger=None):
        self.log = logger
        if self.log is None:
            self.log = logging.getLogger('record-gui')  # internal logger
    ...
    def save_step(self, description, state=None):
        ...
        self.log.debug("Saving step: %s" % description)
        if not state:
    ...
    def undo(self, amount=1):
        ...
        if amount > len(self.steps):
            self.log.warning("I cannot undo what did not happen!")
            return False
        if amount == 1:
            state, description = self.steps.pop()
            self.redo_steps = [(state, description, self.current)]
            self.current = state
            self.log.info("Undoing: %s" % description)
            if self.steps:
                state, description = self.steps[-1]
                self.log.info("Last noted action: %s" % description)
            else:
                self.log.info("There are no previously noted steps")
            self.gui.display_keyframes(self.current.anim)
            return True
        else:
            self.log.info("Undoing %i steps" % amount)
    ...
    def redo(self, amount=1):
        ...
        if not self.redo_steps:
            self.log.warning("Cannot redo what was not undone!")
            return False
        if amount < 0:
            self.log.warning("Amount cannot be negative! (What where you even thinking?)")
            return False
        ...
        self.gui.display_keyframes(self.current.anim)
        self.log.info("Last noted step is now: %s " % self.steps[-1][1])
        return True

    def set_ipc(self, ipc):
        self.ipc = ipc

    def record(self, pos=None):
        """ Record Command, save current keyframe-data
        """
        frame = {
            "duration": 1,
            "pause": 0.0,
            "goals": dict(self.ipc.get_pose().positions)
        }
        if not pos:
            self.save_step("Appending new keyframe #%i" % len(self.anim))
            self.current.anim.append(frame)
            self.gui.append_keyframe(
                frame)  # performanter als alle keyframes neu anzeigen
        else:
            self.save_step("Inserting new keyframe to position %s" % pos)
            self.current.anim.insert(pos, frame)
            self.gui.display_keyframes(self.current.anim)
        return True
...
class JointManager(object):
    def __init__(self, ipc, logger=None):
        self.ipc = ipc
        self.log = logger
        if self.log is None:
            self.log = logging.getLogger('record-gui')  # internal logger
    ...
    def on(self, tag=None):
        positions = dict(self.ipc.get_pose().positions)
        if tag:
            tag = str(tag)
        ...
        selected = joints.get_joints(tag)
        if not selected:
            self.log.warn("No Joints found under the tag %s" % tag)
            return False
        for joint in selected:
            pose[joint.name].active = True
            pose[joint.name].goal = positions[joint.name]

        self.ipc.update(pose)
        return True

    def off(self, tag=None):
        pose = Pose()
        if tag:
            tag = str(tag)
        selected = joints.get_joints(tag)
