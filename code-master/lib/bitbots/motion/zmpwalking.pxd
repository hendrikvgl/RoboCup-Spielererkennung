...
from eigen cimport Vector4d

cdef extern from "zmp_walk.hpp":

    cdef enum FootPhase "ZMPWalk::FootPhase":
        left, right, stopped, both

    cdef cppclass _ZMPWalk "ZMPWalking::ZMPWalk":
        _ZMPWalk(const _ZMPParameter& p)
        CPose& get_pose()
        ...
        void set_tiltX(double tiltX, double TiltX_longTime)
