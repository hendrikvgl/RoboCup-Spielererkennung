...
ZMPWalking::ZMPParameter::ZMPParameter()
    ...
    //---------------------------------------------
    bodyHeight(0.295),
    bodyTilt(25* degree_to_rad),//*pi/180 //es gibt auch ein zmpBodytilt, es müssen wahrscheinlich immer beide geändert werden // Wird vom hipPitch überschrieben
    footX(-0.020),//Verschiebt wohl den Torso am Ende des Updates nach hinten
    footY(0.035),
    supportX(0),
    supportY(0.010),
    ...
    supportFront(0.03), //Lean back when walking fast forward
    ...
    //Verschiebt wohl den Torso nach Vorne, wenn schneller als 0.4
    frontComp(0.003), //standartwert aus awesomewlk.lua weil nciht in der config
    //Verschiebt wohl den Torso nach Vorne, wenn beschleunigung größer als 0.2
    AccelComp(0.003), //standartwert aus awesomewlk.lua weil nciht in der config
