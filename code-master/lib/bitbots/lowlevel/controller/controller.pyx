...
cdef StatusPacket make_status_packet(ubyte *bytes, int n):
    ''' Parsed ein StatusPacket aus dem Speicherbereich '''
    if n < 6:
        raise IOError("Can not decode Packet")

    if bytes[0] != 0xff or bytes[1] != 0xff:
...
cdef class Controller:
    ...
    cdef list read_packets(self, int count, int bytecount):
        ...
        """
        cdef bytes pystr = self.serial.read(bytecount)
        ...
        while viewstart + 6 <= bytecount:
            ...
            #print [hex(x) for x in data[viewstart:viewstart+size]]
            ...
            if checksum(data + viewstart, size) != 0:
                ...
                debug.warning("Checksum mismatch")
                viewstart += 1
                continue

            packet = make_status_packet(data + viewstart, size)
