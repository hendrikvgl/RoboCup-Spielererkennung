...
class DynamicKick(AbstractDecisionModule):
    ...
    def perform(self, connector, reevaluate=False):
        ...
        if self.run == 0:
            ...
            args = ([self.root, self.end_joint, [(1, 0, 0), np.array((0, self.y1, -280))], (1e-2, 1), (0, 3), 100,
                     self.angle_task_joints, self.ignore_joints], 0)
            return self.push(KinematicTask, args)
        elif self.run == 2:
            # 2. kinematische Position
            args = [[self.root, self.end_joint, [(1, 0, 0), np.array((self.x, self.y2, -240))], (1e-2, 1), (0, 3), 100,
                     self.angle_task_joints, self.ignore_joints], 0]
