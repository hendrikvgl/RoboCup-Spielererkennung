...
TEST(Robot, fillJacobiMatrixWithModifiedAxis) {
    ...
    Array3d diff = (ja - ref).array().abs();
    ASSERT_LT(diff.maxCoeff(), 1e-10);

    fill_jacobi_matrix_l(ja, c, AxisType::YAxis);
    ref << 0, 0, 0;
    diff = (ja - ref).array().abs();
...
TEST(Robot, inverseChainWithAxis) {
    RobotParameter p = get_advanced_robot_parameter_with_3_joints();
    JointParameter jp = get_default_joint_parameter();
    jp.rotations = Vector3d(0, 90, 0);
    jp.transform_from_parent = Vector3d(2, 1, 0);
...
TEST(Robot, inverseChainWithPosition) {
    RobotParameter p = get_advanced_robot_parameter_with_3_joints();
    JointParameter jp = get_default_joint_parameter();
    jp.rotations = Vector3d(0, 90, 0);
    ...
    robot.inverse_chain(0, target, (MultipleErrorType(1, 1)<<1e-4).finished(), 100, (MultipleAxisType(1,1)<< axis_t).finished(), 1e-4);
    chain_matrix = robot.get_joint_by_id(2).get_chain_matrix().matrix();
    axis = chain_matrix.col(axis_t).head<3>();
    //There is alway some uncertainty in the results:
...
TEST(Robot, cogJacobiMatrix) {
    RobotParameter p = get_advanced_robot_parameter_with_3_joints();
    JointParameter jp = get_default_joint_parameter();
    jp.masses.clear();
    jp.masses.push_back(Vector4d(0, 0, 0, 1));
    ...
    p.chain_template[1].push_back(2);
    jp.motor_id = 0;
    p.joints[0] = joint_from_parameter(jp);
    jp.motor_id = 2;
    p.joints[2] = joint_from_parameter(jp);
    jp.masses[0] = Vector4d(1, 0, 0, 2);
    jp.transform_from_parent = Vector3d(1000, 0, 0);
    jp.motor_id = 1;
    p.joints[1] = joint_from_parameter(jp);
    Vector2d update = Vector2d::Zero();

    KRobot robot = robot_from_parameter(p);
    robot.update(update);
    JacobiType j(3, 1);
    fill_cog_jacobi_matrix_l(j, robot.get_chain_by_id(0), robot.get_mass());
    Vector3d j3, ref;
    j3 = j.col(0);
    ref<<0, 0, -0.5;
...
TEST(Robot, jacobiFromInverseChain) {
    ...
    KRobot::JacobiType j(3, 1), j_ref(3, 1);
