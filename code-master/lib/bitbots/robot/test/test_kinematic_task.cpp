...
TEST(KinematicTask, CreateSimpleChain) {
    ...
    KinematicTask::ChainHolder holder = task.create_chain(ID(0), ID(2), KJoint::AxisType::Position);
    EXPECT_EQ(*holder, robot.get_chain_by_id(0));

    holder = task.create_chain(ID(0), ID(4), KJoint::AxisType::Position);
    EXPECT_EQ(*holder, robot.get_chain_by_id(1));

    holder = task.create_chain(ID(0), ID(6), KJoint::AxisType::Position);
...
TEST(KinematicTask, CreateParticularChain) {
    ...
    KinematicTask::ChainHolder holder = task.create_chain(ID(1), ID(2), KJoint::AxisType::Position);
    EXPECT_EQ(holder->size(), 2);
    EXPECT_EQ(holder.get()[holder->size() - 1]->get_id(), 2);

    holder = task.create_chain(ID(3), ID(4), KJoint::AxisType::Position);
    EXPECT_EQ(holder->size(), 2);
    EXPECT_EQ(holder.get()[holder->size() - 1]->get_id(), 4);

    holder = task.create_chain(ID(5), ID(6), KJoint::AxisType::Position);
    EXPECT_EQ(holder->size(), 2);
...
TEST(KinematicTask, CompleteInverseChain) {
    ...
    KinematicTask::ChainHolder holder = task.create_chain(ID(2), ID(0), KJoint::AxisType::Position);
    ...
    holder = task.create_chain(ID(4), ID(0), KJoint::AxisType::Position);
    ...
    holder = task.create_chain(ID(6), ID(0), KJoint::AxisType::Position);
...
TEST(KinematicTask, ChainOverRoot) {
    ...
    KinematicTask::ChainHolder holder = task.create_chain(ID(2), ID(4), KJoint::AxisType::Position);
...
TEST(KinematicTask, CoGChainNoChainDisabled) {
    ...
    KinematicTask::ChainHolder holder = task.create_centre_of_gravity_chain(int_to_ChainID(0), set<int>(), KinematicTask::DisableType::none);
...
TEST(KinematicTask, CoGChainInverseChainDisabled) {
    ...
    KinematicTask::ChainHolder holder = task.create_centre_of_gravity_chain(int_to_ChainID(0), set<int>(), KinematicTask::DisableType::inverse_chain);
    KRobot::Chain& chain = *holder;
    unsigned chain_size = 0;
    for(const KRobot::Chain& _chain: robot.get_chains()) {
        chain_size += _chain.size();
...
TEST(KinematicTask, CoGChainDirectChainDisabled) {
    ...
    KinematicTask::ChainHolder holder = task.create_centre_of_gravity_chain(int_to_ChainID(0), set<int>(), KinematicTask::DisableType::direct_chain);
    KRobot::Chain& chain = *holder;
    unsigned chain_size = 0;
    for(const KRobot::Chain& _chain: robot.get_chains()) {
        chain_size += _chain.size();
...
TEST(KinematicTask, ComplexDisableOptions) {
    ...
    KinematicTask::ChainHolder holder = task.create_chain(int_to_JointID(begin), int_to_JointID(end), axis, task_ids, set<int>(), KinematicTask::DisableType::none);
    KRobot::Chain& chain = *holder;
