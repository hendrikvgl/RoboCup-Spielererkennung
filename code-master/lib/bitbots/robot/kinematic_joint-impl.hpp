...
m_mass_offset(Vector::Zero()),m_mass_endpoint(Vector::Zero()),m_chain_masspoint(Vector::Zero()), m_angle(0), m_min_angle(0),m_max_angle(0), m_motor_id(-1)
{}
...
bool KJoint::all_data_valid() const {
    ...
    return !(isnan(m_angle) || isnan(m_max_angle) || isnan(m_min_angle) || hasNaN(m_chain_masspoint) || hasNaN(m_chain_matrix) || hasNaN(m_transform)
...
BITBOTS_INLINE const Eigen::Vector3d KJoint::get_local_gradient(const Vector3& endpoint, const AxisType axis) const {
    ...
    if(axis == AxisType::Position) {
        ...
        if(isnan(delta(0))) delta = Vector3::Zero();
    }
