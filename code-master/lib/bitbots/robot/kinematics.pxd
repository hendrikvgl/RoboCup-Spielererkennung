...
cimport numpy as np
from boost cimport reference_wrapper
...
cdef extern from "kinematic_robot.hpp":
    ...
    cppclass _Robot "::Robot::Kinematics::KRobot":
        ...
        int inverse_local_cog_chain(Chain& chain, Vector3d& target, const Vector3i& ignore_axis, double error, int max_it)
        int inverse_local_cog_chain(Chain& chain, Vector3d& target, const Vector3i& ignore_axis, double error, int max_it, double min_angle_d)
        int inverse_global_cog_chain(Chain& chain, Vector3d& target, const Vector3i& ignore_axis, double error, int max_it)
        int inverse_global_cog_chain(Chain& chain, Vector3d& target, const Vector3i& ignore_axis, double error, int max_it, double min_angle_d)
...
cdef extern from "kinematic_task.hpp":
    ...
    cdef enum _DisableType "Robot::Kinematics::KinematicTask::DisableType":
        none, direct_chain, inverse_chain

    cppclass _KinematicTask "Robot::Kinematics::KinematicTask":

        _KinematicTask(const _Robot&)
        void perform_task(JointIds fromm, JointIds to, Vector3d target, AxisType axis, double error)
        void perform_task(JointIds fromm, JointIds to, Vector3d target, AxisType axis, double error, int max_it)
        void perform_task(JointIds fromm, JointIds to, Vector3d target, AxisType axis, double error, int max_it, set[int] ignore_joints)
        void perform_task(JointIds fromm, JointIds to, Matrix3_2d target, MAxisType axis, Vector2d error, set[unsigned int] first_taks_ids)
        void perform_task(JointIds fromm, JointIds to, Matrix3_2d target, MAxisType axis, Vector2d error, set[unsigned int] first_taks_ids, int max_it)
        void perform_task(JointIds fromm, JointIds to, Matrix3_2d target, MAxisType axis, Vector2d error, set[unsigned int] first_taks_ids, int max_it, set[int] ignore_joints)
        void perform_centre_of_gravity_task(ChainIds fromm, const Vector3d& target, set[int] ignore_joints, double error, int max_it)
        void perform_centre_of_gravity_task(ChainIds fromm, const Vector3d& target, set[int] ignore_joints, double error, int max_it, _DisableType disable_chain)
        _ChainHolder create_chain(JointIds fromm, JointIds to, AxisType axis, set[int] ignore_joints)
        _ChainHolder create_chain(JointIds fromm, JointIds to, MAxisType axis, set[unsigned int] first_taks_ids, set[int] ignore_joints)
        _ChainHolder create_chain(JointIds fromm, JointIds to, MAxisType axis, vector[set[uint]] idset_for_tasks, set[int] ignore_joints)
        _ChainHolder create_centre_of_gravity_chain(ChainIds chain_id, set[int] ignore_joints, _DisableType disable_chain)
        void perform_task(_ChainHolder chain, Vector3d target, AxisType axis, double error, int max_it)
        void perform_task(_ChainHolder chain, Matrix3_2d target, MAxisType axis, Vector2d error, int max_it)
        void update_robot_chain(_ChainHolder chain)
        void update_robot_chain(_ChainHolder chain, unsigned int update_flags)

cdef _DisableType directChain = direct_chain
...
cdef class Robot:
    ...
    cpdef inverse_chain_t(self, int id, object target, float error=?, int max_it=?, int axis=?)
    cpdef inverse_chain(self, int id, object, object error=?, int max_it=?, object axis=?, bool multiple=?)
    cpdef inverse_chain_m(self, int id, list targets, tuple errors, int max_it, tuple axis)
    cpdef inverse_cog_chain(self, int id, object target, object ignore_axis=?, float error=?, int max_it=?, bool local=?)
    cpdef inverse_cog_chain_t(self, int id, object target, object ignore_axis=?, float error=?, int max_it=?, bool local=?)
    cpdef np.ndarray get_centre_of_gravity(self, bool with_mass=?)
...
cdef class KinematicTask:
    ...
    cpdef perform(self, int fromm, int to, object target, object error=?, object axis=?, int max_it=?, list first_task_ids=?, list ignore_joints=?)
    cpdef ChainHolder create_chain(self, int fromm, int to, object axis=?, list first_task_ids=?, list ignore_joints=?)
    cpdef ChainHolder create_cog_chain(self, int chain_id, list ignore_joints=?)
    cpdef perform_h(self, ChainHolder chain, object target, object error=?, object axis=?, int max_it=?)
