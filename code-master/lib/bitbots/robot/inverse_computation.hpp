 ...
 */
static BITBOTS_INLINE MatrixReturnType pseudo_inverse_r(const JacobiType& j, double epsilon=default_epsilon) {
    typedef Eigen::MatrixXd Matrix;
    const Matrix jjt = j * j.transpose();
    Matrix inverse;
    bool success;
...
static BITBOTS_INLINE MatrixReturnType pseudo_inverse(const JacobiType& j, double epsilon=__intern::default_epsilon) {
    if(! minimal_matrix) {
        ...
        if(begin || end != (unsigned)j.cols() - 1) {
            return (MatrixReturnType(j.cols(), j.rows())<<Eigen::MatrixXd::Zero(begin, j.rows()), pseudo_inverse<true>(j.block<Eigen::Dynamic, Eigen::Dynamic>(0, begin, j.rows(), end - begin + 1), epsilon), Eigen::MatrixXd::Zero(j.cols() - end  - 1, j.rows())).finished();
        }
    }
    if(j.cols() < j.rows()) {
        ...
        return __intern::pseudo_inverse_r(j,epsilon);
    }
}
