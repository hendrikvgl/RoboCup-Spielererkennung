...
class RobotHorizonParticleFilter(object):

    def __init__(self):

        self.robot = FeaturedParticle(0, 1500, 45)

        self.particles = [FeaturedParticle(random.randint(-3000, 3000), random.randint(0, 4500), random.randint(0, 360)) for i in range(PARTICLE_COUNT)]
    ...
    def calculate_distances_for_defined_particle(self, particle, noise=None):
        ...
        x = self.robot.x + math.cos((2*math.pi*self.robot.orientation)/360.0) * 100
        y = self.robot.y + math.sin((2*math.pi*self.robot.orientation)/360.0) * 100

        robot_line = self.line([self.robot.x, self.robot.y], [x, y])



        plt.plot([self.robot.x, x], [self.robot.y, y], 'r-')

        L1 = self.line([-3000, 4500], [3000, 4500])
        L2 = self.line([-3000, -4500], [3000, -4500])
        L3 = self.line([ 3000, -4500], [3000, 4500])
        L4 = self.line([-3000, -4500], [-3000, 4500])

        a = self.make(robot_line, L1)
        b = self.make(robot_line, L2)
        c = self.make(robot_line, L3)
        d = self.make(robot_line, L4)

        x = math.cos(2*math.pi*self.robot.orientation/360.0) * 4242
        y = math.sin(2*math.pi*self.robot.orientation/360.0) * 4242

        print "dst"
        print self.dst(a, (self.robot.x, self.robot.y))
        print self.dst(b, (self.robot.x, self.robot.y))
        print self.dst(c, (self.robot.x, self.robot.y))
        print self.dst(d, (self.robot.x, self.robot.y))

        print "dsttotg"
        print self.dst(a, (x,y))
        print self.dst(b, (x,y))
        print self.dst(c, (x,y))
        print self.dst(d, (x,y))
...
angle_matching = RobotHorizonParticleFilter()
