...
class CircleFactory(object):
    ...
    def sample_circles(self, number, reachability_matrix):
        circles = []
        while len(circles) < number:
            i = random.randint(0, self.num_points-1)
            j = random.randint(0, self.num_points-1)
            k = random.randint(0, self.num_points-1)

            if math.isinf(reachability_matrix[i][j]) or math.isinf(reachability_matrix[j][k]):
                continue

            p1, p2, p3 = sorted([self.points[i], self.points[j], self.points[k]])
    ...
    def estimate_quality(self, circle, test_points):
        error = [math.sqrt((self.dst(pnt, (circle.xc, circle.yc)) - circle.r)**2) for pnt in test_points]
        print np.array(error).var()
        circle.quality = np.array(error).var()
...
for remaining_edge in edge_heap:
    i, j = remaining_edge[1]
    plt.plot([P[i][0], P[j][0]], [P[i][1], P[j][1]], c='r')



plt.figure(15)
c = plt.imshow(K, interpolation='none')
plt.colorbar(c)


circle_factory = CircleFactory(points)
circles = circle_factory.sample_circles(number=200, reachability_matrix=K)
...
circle_factory.test_circle_quality(circles, num_points=len(points)/2)
